# 语言特性 × 语义映射（Go 1.26 × OTLP）

本文聚焦 Go 1.26 的关键语言/标准库演进，如何承载 OTLP 语义模型并在 CSP 并发范式下保持语义完整。

## 📋 目录

- [语言特性 × 语义映射（Go 1.26 × OTLP）](#语言特性--语义映射go-1251--otlp)
  - [📋 目录](#-目录)
  - [Go 1.26 要点](#go-1251-要点)
    - [运行时优化](#运行时优化)
    - [内存模型与并发](#内存模型与并发)
...
- **Green Tea GC（Go 1.26 默认启用）**：全新垃圾收集器，小对象扫描性能提升 10-40%，特别适合OTLP高频Span创建场景。旧版需 `GOEXPERIMENT=greenteagc`，Go 1.26 已默认启用
