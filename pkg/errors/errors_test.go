...
func TestWrap(t *testing.T) {
	...
	t.Run("wrap OTLPError preserves chain", func(t *testing.T) {
		...
		assert.Equal(t, CodeNotFound, GetCode(wrapped))
	})
}
...
func TestIsRetryable(t *testing.T) {
	...
	t.Run("default retryable codes", func(t *testing.T) {
		assert.True(t, IsRetryable(New("test").WithCode(CodeUnavailable)))
		assert.True(t, IsRetryable(New("test").WithCode(CodeResourceExhausted)))
		assert.True(t, IsRetryable(New("test").WithCode(CodeAborted)))
	})
