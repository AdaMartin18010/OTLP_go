...
│   └── span_pooling: 319 行
│
├── 弹性设计 (resilience/): 450 行
│   └── circuit_breaker: 450 行
│
├── 自定义处理器 (processor/): 380 行
│   └── custom_processor: 380 行
│
├── 基准测试 (benchmarks/): 320 行
│   └── performance_test: 320 行
│
├── 示例代码 (examples/): 292 行
│   └── context_baggage: 292 行
│
└── 主程序: 1,108 行
    ├── main: 250 行
    ├── metrics: 61 行
...
| 类型 | 数量 | 说明 |
|------|------|------|
| **Go 源文件** | 27 | 包含所有 .go 文件 |
| **Markdown 文档** | 38 | 理论 + 实现 + 项目文档 |
| **TLA+ 规约** | 2 | 形式化验证 |
| **配置文件** | 3 | go.mod, README, LICENSE |
| **总文件数** | 70+ | 完整项目文件 |
